        ...
        {wishlistItems.map(product => (
          <div key={product.id} style={{ border: '1px solid var(--color-separator)', padding: '20px', position: 'relative' }}>
            ...
            <Link to={`/product/${product.id}`}>
              <img src={product.images[0]} alt={product.name} style={{ width: '100%', height: '300px', objectFit: 'cover', marginBottom: '15px' }} />
              <h3 style={{ fontSize: '14px', marginBottom: '5px', color: 'var(--color-body-text)', textTransform: 'none' }}>{product.name}</h3>
              <div style={{ fontFamily: 'var(--font-heading)', fontSize: '16px', color: 'var(--color-heading-text)', marginBottom: '15px' }}>
